...
class DataLoader:
            ...
            with open(csv_path, "r", encoding="utf-8") as csvfile:
                ...
                for row_num, row in enumerate(
                    ...
                    try:
                        ...
                        customer_name = DataHelpers.clean_string(
                        ...
                        session.rollback()
                        Logger.log_error(
                            logger, e, f"Error processing customer at row {row_num}"
                        )
                        customers_skipped += 1
                        continue
                ...
                session.commit()
                logger.info(
                    f"Customers loaded: {customers_loaded}, skipped: {customers_skipped}"
                )
        ...
        try:
            ...
            for order_elem in root.findall("order"):
                try:
                    ...
                    order_id = DataHelpers.safe_int(
                        order_elem.find("order_id").text
                        if order_elem.find("order_id") is not None
                        else None
                    )
                    mobile_number = DataHelpers.normalize_mobile_number(
                        order_elem.find("mobile_number").text
                        if order_elem.find("mobile_number") is not None
                        else None
                    )
                    order_date_str = (
                        order_elem.find("order_date_time").text
                        if order_elem.find("order_date_time") is not None
                        else None
                    )
                    order_date_time = DataHelpers.normalize_date(order_date_str)
                    sku_id = DataHelpers.clean_string(
                        order_elem.find("sku_id").text
                        if order_elem.find("sku_id") is not None
                        else None
                    )
                    sku_count = DataHelpers.safe_int(
                        order_elem.find("sku_count").text
                        if order_elem.find("sku_count") is not None
                        else None
                    )
                    total_amount = DataHelpers.safe_float(
                        order_elem.find("total_amount").text
                        if order_elem.find("total_amount") is not None
                        else None
                    )
                    ...
                    session.rollback()
                    Logger.log_data_quality_issue(
                        logger, "Duplicate order ID", {"order_id": order_id}
                    )
                    orders_skipped += 1
                    continue
                except Exception as e:
    ...
    def load_all_data(self) -> Dict[str, int]:
        ...
        summary = {"customers_loaded": customers_count, "orders_loaded": orders_count}
