...
class PandasAnalytics:
    ...
    def load_customers_from_csv(self, csv_path: str) -> pd.DataFrame:
        ...
        df["mobile_number"] = df["mobile_number"].apply(
            DataHelpers.normalize_mobile_number
        )
        ...
        if dropped_count > 0:
            logger.warning(
                "Dropped {} rows with missing required fields".format(dropped_count)
            )
    ...
    def load_orders_from_xml(self, xml_path: str) -> pd.DataFrame:
        ...
        df["total_amount"] = df["total_amount"].apply(
            lambda x: DataHelpers.safe_float(x)
        )
    ...
    def get_repeat_customers(
        ...
        repeat_customers = df_customers.merge(
            repeat_order_counts, on="mobile_number", how="inner"
