...
class SQLAnalytics:
            ...
            monthly_trends = [
                {
                    ...
                    "order_count": r.order_count,
                    "total_revenue": round(
                        float(r.total_revenue) if r.total_revenue else 0, 2
                    ),
                }
                for r in results
            ]
    ...
    def get_regional_revenue(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = (
                session.query(
                    Customer.region,
                    func.count(func.distinct(Customer.customer_id)).label(
                        "customer_count"
                    ),
                    func.count(Order.order_id).label("order_count"),
                    func.sum(Order.total_amount).label("total_revenue"),
            ...
            regional_revenue = [
                {
                    ...
                    "avg_order_value": round(
                        float(r.avg_order_value) if r.avg_order_value else 0, 2
                    ),
                }
                for r in results
            ]
            ...
            logger.info(
                "Found revenue data for {} regions".format(len(regional_revenue))
            )
    ...
    def get_top_spenders(self, days: int = 30, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(
            "Calculating top {} spenders for last {} days using SQL...".format(
                limit, days
            )
        )
        session = self.db_manager.get_session()
            ...
            top_spenders = [
                {
                    ...
                    "order_count": r.order_count,
                    "total_spent": round(
                        float(r.total_spent) if r.total_spent else 0, 2
                    ),
                    "avg_order_value": round(
                        float(r.avg_order_value) if r.avg_order_value else 0, 2
                    ),
                    "last_order_date": r.last_order_date.strftime("%Y-%m-%d")
